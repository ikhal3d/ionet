...
(function (global) {
  ...
  global.buildTopology = function (host) {
    ...
    function fitTopoSize(el) {
      var xfm = window.getComputedStyle(el).transform;
      if (xfm && xfm !== 'none') return; // CSS scale handles tablet range
      var pw = el.parentElement.offsetWidth;
      if (pw > 0 && pw < 660) el.style.zoom = (pw / 660).toFixed(4);
      else el.style.zoom = '';
    }
    fitTopoSize(host);
    window.addEventListener('resize', function () { fitTopoSize(host); }, { passive: true });
  };
})(window);
