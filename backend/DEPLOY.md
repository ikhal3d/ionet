...
| Cloudflare **API Token** (Workers edit) | Private | 2026-04-29 | not stored on disk — exported as `CLOUDFLARE_API_TOKEN` only when running `wrangler deploy` | quarterly · or revoke immediately after each deploy if you prefer | 2026-04-29 | revoke if leaked — does NOT touch DNS, only Workers + Routes |
